...
```c
lv_obj_set_x(back_btn, 30);
lv_obj_set_y(back_btn, 10);
...
```c
lv_obj_add_style(back_btn, &style1, LV_STATE_PRESSED); /*Equal to LV_PART_MAIN | LV_STATE_PRESSED*/
```
...
```c
lv_obj_add_style(back_btn, &style1, 0); /*Equal to LV_PART_MAIN | LV_STATE_DEFAULT*/
```
...
lv_obj_add_style(back_btn, &style1_btn_red, 0);
```
